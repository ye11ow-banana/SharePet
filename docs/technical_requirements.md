...
Пользователь откликается на других животных, переписывается с их хозяевами, редактирует профиль, изменяет настройки, читает, комментирует, лайкает и создает блоги.

Администратор проверяет блоги и комментарии на соблюдение правил пользования, а также проверяет чаты при жалобе одного из участников.

Анонимный пользователь читает блоги.

Руководство имеет доступ к базе данных и создает ссылки для регистрации новых администраторов.
...
3. Добавление животных к аккаунту
4. Нахождения пары (доступно только с галочкой “проверено”)
5. Чат с хозяином понравившегося животного (после взаимных лайков)
6. Расчет и безопасная сделка (с согласия двух сторон)
7. Заведение и редактирование контента блога
8. Взаимодействие с блогами
...
В этом разделе пользователь может написать статью. В статье он может вставить картинку или прикрепить файл. В любой момент может изменить контент статьи.
...
1. Бан статей
2. Бан пользователей
3. Просмотр чата при жалобе одного из участников
4. Вынесение решения при спорах в сделке
5. Проверка нового животного
